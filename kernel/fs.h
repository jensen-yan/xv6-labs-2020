...
#define NDIRECT 11    // 最大11个直接块
#define NINDIRECT (BSIZE / sizeof(uint))    // 256个间接块
#define NDOUINDIRECT (NINDIRECT * NINDIRECT)  // 256 * 256 个双重间接
...
struct dinode {
  ...
  uint addrs[NDIRECT+2];   // Data block addresses 文件存放在数据块上的位置数组, 加上了双重间接
};
